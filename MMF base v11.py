...
def check_valid_age(minium, maximum):
    ...
    valid_age = check_valid_age(f"Please confirm {name}'s age: ")
    if valid_age < minium:
        print(f"sorry {name} is too young for this movie")
    else:
        while not valid_age <= maximum:
            valid_age = check_valid_age(f"at {age} {name} is very old, please re-enter {name}'s age")
        return valid_age
